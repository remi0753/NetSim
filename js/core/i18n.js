...
(function (root) {
  ...
  const D = {
    ...
    'insp.link.type':      { ja: 'UTPケーブル (全二重)', en: 'UTP cable (full-duplex)' },
    ...
    'insp.ospf.note':      { ja: '設定はCLI: <code>router ospf 1</code> → <code>network 10.0.0.0 0.255.255.255 area 0</code><br>確認: <code>show ip ospf neighbor</code> / <code>show ip route</code>', en: 'Configure in the CLI: <code>router ospf 1</code> → <code>network 10.0.0.0 0.255.255.255 area 0</code><br>Check: <code>show ip ospf neighbor</code> / <code>show ip route</code>' },
