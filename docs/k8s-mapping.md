...
(ダブルクリック)で以下を打つ。ポートは既定 8 ポート。**L3スイッチ/L2スイッチのポートは
Gi0/1〜Gi0/8(1始まり、Gi0/0 は存在しない)**、ルータのみ Gi0/0 始まり。
各ノードは Gi0/1 をアップリンク、Gi0/2 以降を Pod 用とする。
...
Gi0/1=アップリンク(VLAN90)、Gi0/2〜=Pod(既定VLAN1)。
```
enable
configure terminal
 interface Gi0/1
...
### 障害と収束(ノードダウン)
- Node2 のアップリンク(Gi0/1)を `shutdown` → OSPF が `10.244.2.0/24` を失効
...
- **物理サーバ層と外部GWを足した応用構成** → [k8s-multihost.md](k8s-multihost.md)。
  1台の host server にノードVMを2つ載せ、host server 3台で6ノード=1クラスタを組み、
  さらにクラスタと別ネットワークの出入り口(egress SNAT / ingress=LoadBalancer)を境界ルータで再現する。
  ノードローカリティ・ホスト障害ドメインまで観察できる。
